...
export const zh: Translations = {
  ...
  settings: {
    ...
    skillsDirs: {
      title: '技能目录',
      blurb: 'Hermes 扫描技能的目录。附加目录在内置目录之外扩展技能来源。',
      count: n => `+${n} 个附加`,
      builtinBadge: '内置（可写）',
      add: '添加目录…',
      addPickerTitle: '选择技能目录',
      open: '打开',
      remove: '移除',
      missingBadge: '目录缺失',
      cleanupMissing: '清理失效目录',
      empty: '尚未配置附加目录。',
      added: dir => `已添加 ${dir}，对新会话生效。`,
      removed: dir => `已移除 ${dir}，对新会话生效。磁盘内容未被删除。`,
      cleanedUp: n => `已清理 ${n} 个失效目录。`,
      addDuplicate: '该目录已在列表中。',
      addBuiltinConflict: '该目录即为内置技能目录。',
      addNotFound: '目录不存在。',
      saveFailed: '保存技能目录失败',
    ...
    screenshots: {
      title: '截图',
      blurb: '快速截图与粘贴图片的存储位置，以及占用的空间。',
      dirModeTitle: '保存位置',
      dirModeDesc: '项目模式把截图保存在工作区内，agent 之后可按路径复读图片。只影响之后的截图。',
      dirModeAppData: '应用数据目录（默认）',
      dirModeProject: '当前项目（.hermes/screenshots）',
      projectNoWorkspace: '当前没有工作区——项目模式暂时回退到应用数据目录。',
      gitExcludeTitle: '加入 git 排除',
      gitExcludeDesc: '把 .hermes/screenshots/ 追加到该仓库的 .git/info/exclude（不动 .gitignore）。对当前工作区生效。',
      gitExcludeApplied: '已写入 .git/info/exclude。',
      gitExcludeAlready: '已在排除列表中。',
      gitExcludeFailed: '写入 git 排除失败',
      storageTitle: '存储占用',
      storageFiles: (files, size) => `${files} 个文件 · ${size}`,
      storageEmpty: '空',
      storageLoadFailed: '读取截图存储统计失败',
      clean: '一键清理',
      cleaning: '正在清理…',
      cleanConfirm: '删除两个目录中的全部截图？历史消息中的图片附件引用将失效。',
      cleaned: (files, size) => `已删除 ${files} 个文件，释放 ${size}。`,
      cleanFailed: '清理失败',
      overlayTitle: '截图悬浮按钮',
      overlayDesc: '显示一个可随意拖动的置顶小圆钮，点击即可截取窗口或整个屏幕。'
    },
    plugins: {
      title: '桌面插件',
      blurb:
  ...
  composer: {
    ...
    screenshot: {
      ...
      pickWindow: '截取窗口',
      entireScreen: '整个屏幕',
      loadingWindows: '正在加载窗口…',
      noWindows: '未找到可截取的窗口。',
      failedTitle: '截图失败',
      annotateTitle: '标注截图',
      annotateHint: '在截图上直接框选或标记——每个标注自动编号，说明填在右侧。',
      notesTitle: '标注说明',
      notesEmpty: '先在图上画一个标注。',
      notePlaceholder: label => `${label} 的说明…`,
      noteLabel: (image, mark) => `图${image}:${mark} `,
      toolRect: '矩形',
      toolArrow: '箭头',
      toolPen: '画笔',
      undo: '撤销',
